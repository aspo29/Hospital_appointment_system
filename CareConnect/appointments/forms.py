...
class AppointmentForm(forms.ModelForm):
    ...
    appointment_date = forms.DateField(initial=date.today, widget=forms.SelectDateWidget(years=range(date.today().year, date.today().year + 1), attrs={'class': 'form-select mx-2', 'style': 'width: 29%; display: inline-block;'}), required=True, label='Date')
    appointment_time = forms.ChoiceField(required=True, label='Time', widget=forms.Select(attrs={'class': 'form-select'}))
    price_category = forms.ChoiceField(required=True,label='Price',choices=Appointment.PRICE_CATEGORY_CHOICES,widget=forms.Select(attrs={'class': 'form-select'}))
    ...
    class Meta:
        model = Appointment
        fields = ['doctor', 'specialty', 'appointment_date', 'appointment_time','price_category', 'symptom_description', 'payment_method']
